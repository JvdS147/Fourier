...
void interpret_f0_file( const FileName & input_file_name, const FileName & output_file_name )
{
    ...
    comment_identifiers.push_back( "#N 9" );
    ...
    // Fill Deuterium, which is not in the input file, with dummy values.
    // These dummy values will be replaced by the values for Hydrogen upon writing the output file.
    a1.push_back( "0.0" );
    a2.push_back( "0.0" );
    a3.push_back( "0.0" );
    a4.push_back( "0.0" );
    ...
    b3.push_back( "0.0" );
    b4.push_back( "0.0" );
    int previous_atomic_number( 0 );
    while ( text_file_reader.get_next_line( words ) )
    {
        if ( words.size() != 3 )
            throw std::runtime_error( "Could not read line 1b" );
        if ( words[0] != "#S" )
            throw std::runtime_error( "Could not read line 1c" );
        atomic_number = string2integer( words[1] );
        Element element( atomic_number );
        if ( element.symbol() != words[2] )
            throw std::runtime_error( "Element inconsistent" );
        if ( atomic_number != (previous_atomic_number + 1) )
            throw std::runtime_error( "Elements not consecutive" );
        previous_atomic_number = atomic_number;

//        if ( ! text_file_reader.get_next_line( words ) )
//            throw std::runtime_error( "Could not read line 2" );
//        if ( line != "#N 9")
//            throw std::runtime_error( line + " != #N 9" );
        if ( ! text_file_reader.get_next_line( words ) )
            throw std::runtime_error( "Could not read line 3" );
        if ( text_file_reader.get_line() != "#L a1  a2  a3  a4  c  b1  b2  b3  b4")
            throw std::runtime_error( text_file_reader.get_line() + " != #L a1  a2  a3  a4  c  b1  b2  b3  b4" );
        if ( ! text_file_reader.get_next_line( words ) )
            throw std::runtime_error( "Could not read line 4" );
        if ( words.size() != 9 )
            throw std::runtime_error( "Number of coefficients != 9" );
        a1.push_back( words[0] );
        a2.push_back( words[1] );
        a3.push_back( words[2] );
        a4.push_back( words[3] );
         c.push_back( words[4] );
        b1.push_back( words[5] );
        b2.push_back( words[6] );
        b3.push_back( words[7] );
        b4.push_back( words[8] );
    }

    TextFileWriter output_file( output_file_name );
    size_t i;

    output_file.write_line( "static const double a1[113] =" );
    output_file.write_line( "                   {" );
    // Deuterium is number 0, we give it the values from hydrogen, number 1:
    output_file.write( "                       " + a1[1] + ", " );
    i = 1;
    for ( ; i != a1.size(); ++i )
    {
        output_file.write( a1[i] + ", " );
        if ( ( ( i + 1 ) % 7 ) == 0 )
        {
            output_file.write_line();
            output_file.write( "                       " );
    ...
    output_file.write_line( "static const double a2[113] =" );
    output_file.write_line( "                   {" );
    // Deuterium is number 0, we give it the values from hydrogen, number 1:
    output_file.write( "                       " + a2[1] + ", " );
    i = 1;
    for ( ; i != a2.size(); ++i )
    {
        output_file.write( a2[i] + ", " );
        if ( ( ( i + 1 ) % 7 ) == 0 )
        {
            output_file.write_line();
            output_file.write( "                       " );
    ...
    output_file.write_line( "static const double a3[113] =" );
    output_file.write_line( "                   {" );
    // Deuterium is number 0, we give it the values from hydrogen, number 1:
    output_file.write( "                       " + a3[1] + ", " );
    i = 1;
    for ( ; i != a3.size(); ++i )
    {
        output_file.write( a3[i] + ", " );
        if ( ( ( i + 1 ) % 7 ) == 0 )
        {
            output_file.write_line();
            output_file.write( "                       " );
    ...
    output_file.write_line( "static const double a4[113] =" );
    output_file.write_line( "                   {" );
    // Deuterium is number 0, we give it the values from hydrogen, number 1:
    output_file.write( "                       " + a4[1] + ", " );
    i = 1;
    for ( ; i != a4.size(); ++i )
    {
        output_file.write( a4[i] + ", " );
        if ( ( ( i + 1 ) % 7 ) == 0 )
        {
            output_file.write_line();
            output_file.write( "                       " );
    ...
    output_file.write_line( "static const double b1[113] =" );
    output_file.write_line( "                   {" );
    // Deuterium is number 0, we give it the values from hydrogen, number 1:
    output_file.write( "                       " + b1[1] + ", " );
    i = 1;
    for ( ; i != b1.size(); ++i )
    {
        output_file.write( b1[i] + ", " );
        if ( ( ( i + 1 ) % 7 ) == 0 )
        {
            output_file.write_line();
            output_file.write( "                       " );
    ...
    output_file.write_line( "static const double b2[113] =" );
    output_file.write_line( "                   {" );
    // Deuterium is number 0, we give it the values from hydrogen, number 1:
    output_file.write( "                       " + b2[1] + ", " );
    i = 1;
    for ( ; i != b2.size(); ++i )
    {
        output_file.write( b2[i] + ", " );
        if ( ( ( i + 1 ) % 7 ) == 0 )
        {
            output_file.write_line();
            output_file.write( "                       " );
    ...
    output_file.write_line( "static const double b3[113] =" );
    output_file.write_line( "                   {" );
    // Deuterium is number 0, we give it the values from hydrogen, number 1:
    output_file.write( "                       " + b3[1] + ", " );
    i = 1;
    for ( ; i != b3.size(); ++i )
    {
        output_file.write( b3[i] + ", " );
        if ( ( ( i + 1 ) % 7 ) == 0 )
        {
            output_file.write_line();
            output_file.write( "                       " );
    ...
    output_file.write_line( "static const double b4[113] =" );
    output_file.write_line( "                   {" );
    // Deuterium is number 0, we give it the values from hydrogen, number 1:
    output_file.write( "                       " + b4[1] + ", " );
    i = 1;
    for ( ; i != b4.size(); ++i )
    {
        output_file.write( b4[i] + ", " );
        if ( ( ( i + 1 ) % 7 ) == 0 )
        {
            output_file.write_line();
            output_file.write( "                       " );
    ...
    output_file.write_line( "static const double c[113] =" );
    output_file.write_line( "                   {" );
    // Deuterium is number 0, we give it the values from hydrogen, number 1:
